...
def init():
	...
	if reset or not conf:
		...
		# 	return None
		secret_key = input("Super secret key (empty for random key): ")
		if not secret_key:
			import uuid 
			print("Random key chosen.")
	...
	if len(username) == 0 or len(password) == 0 or password != retype_pass:
		...
		with open(path.join("db", "database.db"), "w") as f:
			pass
		from run import init_db, hash_password, connect_db
		init_db()
		db = connect_db()
		try:
			r = db.execute("insert into User(username, password, admin) values (?,?,?)", [username, hash_password(password), True])
